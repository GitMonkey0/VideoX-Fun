...
#     m = IMGNUM.search(p.name)
#     if m:
#         return (0, int(m.group(1)))
#     # fallback：自然排序
#     return (1, [int(x) if x.isdigit() else x.lower() for x in re.split(r"(\d+)", p.name)])

# def is_cam_dir(p: Path) -> bool:
#     return p.is_dir() and p.name.startswith("cam") and any(f.is_file() and f.suffix.lower()==".jpg" for f in p.iterdir())
...
#     lines = []
#     for im in imgs:
#         s = str(im.resolve())          # 关键：绝对路径
#         s = s.replace("'", r"'\''")    # 防单引号
#         lines.append(f"file '{s}'")
#     list_path.parent.mkdir(parents=True, exist_ok=True)
#     list_path.write_text("\n".join(lines) + "\n", encoding="utf-8")

# def images_to_video(cam_dir: Path, out_mp4: Path, fps: float):
#     imgs = sorted([p for p in cam_dir.iterdir() if p.is_file() and p.suffix.lower()==".jpg"], key=img_sort_key)
#     if not imgs:
#         return False

#     list_dir = out_mp4.parent / ".tmp_ffmpeg_lists"
#     list_path = list_dir / (out_mp4.stem + ".txt")
#     write_concat_list(imgs, list_path)
...
#         if not cap.is_dir():
#             continue
#         # src/test/CaptureX/<sequence>/cam*
#         for cam in cap.glob("*/*"):
#             # 这一层是 cam 目录：CaptureX/SEQ/camXXXX
...
#         try:
#             done = images_to_video(cam_dir, out_mp4, args.fps)
#             if done:
#                 ok += 1
...
#             fail += 1
#             print(f"[FAIL] {cam_dir}\n{e}\n")

#     print(f"Done ok={ok} skip={skip} fail={fail} out={dst}")
