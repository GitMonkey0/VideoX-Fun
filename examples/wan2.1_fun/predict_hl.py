...
with torch.no_grad():
    ...
    # 1) 准备 hl_ids（放到 control_video 之后、调用 pipeline 之前）
    ...
    # 对齐到 video_length（注意：这里用的是送入 pipeline 的视频帧数）
    if hl_ids.size(0) > video_length:
        hl_ids = hl_ids[:video_length]
    if hl_ids.size(0) < video_length:
        pad = torch.full((video_length - hl_ids.size(0), hl_ids.size(1)), -1, dtype=torch.long)
        hl_ids = torch.cat([hl_ids, pad], dim=0)

    # batch 维度
    hl_ids = hl_ids.unsqueeze(0)  # (1, F, 40)
